...
(define dark-theme
  (hash
   ...
   'shadows
   (hash 'small (list 0 2 4 0 (hash-ref dark-colors 'shadow-light))
         'medium (list 0 4 12 0 (hash-ref dark-colors 'shadow-medium))
         'large (list 0 8 24 0 (hash-ref dark-colors 'shadow-heavy)))
...
(define (dark-theme-ref key [default #f])
  (hash-ref dark-theme key default))

;; Get a color from the theme
(define (dark-theme-color key [alpha 1.0])
  (let ([color-value (hash-ref (hash-ref dark-theme 'colors) key #f)])
    (if color-value
        (hex->color color-value alpha)
        (error "Color not found in dark theme: ~a" key))))

;; Get a font from the theme
(define (dark-theme-font size-key weight-key [style-key 'normal])
  (let* ([typography (hash-ref dark-theme 'typography)]
